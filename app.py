...
def leer_ticket_con_ia(imagen_pil, api_key):
    try:
        # Ajuste de seguridad y transporte para evitar errores 404/bloqueos
        os.environ["GOOGLE_API_USE_MTLS_ENDPOINT"] = "never"
        genai.configure(api_key=api_key, transport='rest')
        
        # Usamos el modelo flash con el nombre técnico más estable
        model = genai.GenerativeModel('gemini-1.5-flash')
        
        prompt = """
        Eres un analista de calidad de cereales experto. 
        Analiza esta imagen de un ticket de Alimentos Polar y extrae exclusivamente los valores de Humedad e Impurezas.
...
# Inicializar valores de IA para el formulario
...
with st.sidebar:
    ...
    if archivo_img and api_key_input:
        ...
        if st.button("🚀 Extraer Datos con IA"):
            with st.spinner("Analizando calidad con IA..."):
                resultado = leer_ticket_con_ia(img, api_key_input)
                st.info(resultado)
                try:
                    # Limpiamos y extraemos los números para el formulario
                    ...
                    st.warning("No se pudo extraer el número automáticamente. Ingrésalo manualmente abajo.")
...
with st.form("registro", clear_on_submit=True):
    ...
    f1, f2 = st.columns(2)
    # Estos campos se llenan automáticamente cuando la IA termina
    ...
    if st.form_submit_button("✅ Guardar en Lista"):
        ...
        # Resetear datos de IA para el próximo ticket
        st.session_state.datos_ia = {"h": 0.0, "imp": 0.0}
...
if st.session_state.lista_inspecciones:
    ...
    col_pdf, col_clear = st.columns([1, 4])
    with col_pdf:
